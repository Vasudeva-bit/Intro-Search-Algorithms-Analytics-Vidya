...
class quickNode():
    """quickNode is a node that has properties like state, parent, action, cost"""
    def __init__(self, state, parent, action, cost):
        """Used for informed algorithm implementation"""
        ...
        self.cost = cost # stores the heuristics calculated in GBFS and A* algorithms
...
class Frontier():
    ...
    def min_node(self, goal):
        """returns the efficient node to explore"""
        return min(self.frontier, key=lambda node: node.cost+(abs(goal[0]-node.state[0])+abs(goal[1]-node.state[1])))

    def delete(self, node):
        """deletes a specific node from the frontier"""
...
class maze():
    ...
    def solve(self):
        ...
        start = quickNode(state=self.start, parent=None, action=None, cost=0)
        frontier.push(start)
        self.explored = set()
        while True:
            ...
            node = frontier.min_node(self.goal)
            ...
            for action, state in self.neighbors(node.state):
                if not frontier.containsState(state) and state not in self.explored:
                    child = quickNode(state=state, parent=node, action=action, cost=node.cost+1)
                    frontier.push(child)
